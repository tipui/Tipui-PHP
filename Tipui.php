...
class FW
{
	...
	private $core_data_storage_mode;
	...
	*/
	private function SetCoreDataCache( $core, $method )
	{
		...
		{
			$this -> core_data_storage_mode = $core -> $method();
		}else{

			self::$core_cached_data[$method] = $core -> $method();
/*
			$s = new Builtin\Libs\Storage;
			$s -> SetMode( $this -> core_data_storage_mode );
			$s -> SetData( self::$core_cached_data );
